    ...
    public function getPagination($perPage = 10,$separator = ' | ') {
        ...
        for ($i = 0; $i < $pageLinkCount; ++$i) {
            ...
            if ($_GET[$searchOffset] == $pageArray['offset']) {
                ...
                $pageArray['link'] = $this->modx->makeUrl($id, $urlScheme,
                    array($searchOffset => $pageArray['offset'], $searchIndex => $searchString));
    ...
    public function addHighlighting($string, $cls = 'sisea-highlight',$tag = 'span') {
        if (is_array($this->searchArray)) {
            foreach ($this->searchArray as $key => $value) {
                $string = preg_replace('/' . $value . '/i', '<'.$tag.' class="'.$cls.'">$0</'.$tag.'>', $string);
            }
        }
        return $string;
    }
